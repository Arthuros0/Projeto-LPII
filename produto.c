...
void insere_produto(Produto *produto, int *qtdeProd)
{
    ...
    {
        //verifica se o codigo recebido � igual ao de algum produto cadastrado
    ...
    //verifica se houve altera��o no verificador
    //se o c�digo j� correspondia a outro produto, retorno devolve 0
    //e o programa n�o segue com a inser��o de outras informa��es
    if (retorno)
    {
...
void consulta_preco(Produto *produto, int *qtdeProd)
{
    ...
    printf("\n\tDigite o codigo do produto que voc� deseja saber o preco: ");
    scanf("%d", &codigoAux);
...
void mostra_estoque(Produto *produto, int *qtdeProd)
{
    ...
    else
    {
        ...
        for(int i = 0; i <= *qtdeProd; i++)
        {
            ...
            printf("\nPre�o do produto: R$%.2f", produto[i].preco);
